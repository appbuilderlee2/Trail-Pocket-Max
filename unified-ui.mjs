...
const APP_VERSION_FALLBACK='v4.2.7';
const paths = {
...
function applyVisibleVersion(version){
 ...
 const settingsVersion=$('settingsVersion');if(settingsVersion)settingsVersion.textContent=v;
...
function unifyLocationControls(){
 ...
 const normalizeLabel=()=>{
  ...
  const pill=document.querySelector('.settings-state-pill');
  if(pill){pill.textContent=stopping?'● 定位中':'● 尚未定位';pill.classList.toggle('is-active',stopping);}
 };
