...
test('v2 upgrade preserves all offline data and activity completion is atomic',async()=>{
 ...
 await db.put('settings',{id:'active-activity',value:{id:'a'}});await db.finishActivity({id:'a',status:'completed',distance:100});assert.equal((await db.get('activities','a')).distance,100);assert.equal(await db.get('settings','active-activity'),undefined);assert.equal((await db.get('areas','keep')).value,'areas');await db.removeActivity('a');assert.equal(await db.get('activities','a'),undefined);
